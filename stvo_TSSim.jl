...
function find_match_TSArray(ret_stat::String, thisArray::Vector{ChotVDXT})
    ...
    #println("stvo find_match_TSArray... in restat: ", ret_stat, "Len ", length(thisArray))

    for (ind, line) in enumerate(thisArray)
        #println("stvo ==> [", ind,"] ",line)
...
function getAcardFromDeck(name::String)
...
function getPerfectTesthand()

  listName = ["TT", "sT", "tT", "xD", "pD", "mD", "cV", "cT", "cX", "cD", "mT", "mT", "cV","cV", "sX", "sV", "TT", "tT", "TD", "tV"]
...
function stvoClientSim(curCard::Card, TSplayerHand::Deck)

    global TScArray, TSstArray, TSxpmArray


    TuSacCards.ssort(TSplayerHand) # for easy to see all the cards 
        
        println(TSplayerHand)

        (nhand, assets) = find_duplicate(curCard::Card, TSplayerHand::Deck)
        ...
        if(string(curCard)[1] == 'x' || string(curCard)[1] == 'p'|| string(curCard)[1] == 'm')
            ...
            if(index == 0)
                println("stvo==> No match found in file..... HUGE PROBLEM AND IT SHOULD NOT GET HERE! ")
                println(TSxpmArray)
                match_Cards = "N"
                trash_Card  = "N"
                return (match_Cards, trash_Card)
        ...
        if(string(curCard)[1] == 'm') 
            ...
            println("stvo==> match :", match_Cards, "|", trash_Card)
            return (match_Cards, trash_Card)
...
function stv_testModule()
    ...
    global TSstArray  = readTSFile("TS_stText.txt")
    global TSxpmArray = readTSFile("TS_xpmText.txt")
    global TScArray   = readTSFile("TS_cText.txt")


    for n in 1:20

        thisCard = player2_hand.cards[1]

        (matchCards, trasCard) = stvoClientSim(thisCard, player1_hand)

        println("matchCards, trasCard: ", matchCards, " | ", trasCard)
    end
